...
export function CategoryForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setError((await response.json()).error ?? "Unable to create collection.");
      setBusy(false);
      return;
    }
    setBusy(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <label>
          Name
          <input name="name" required placeholder="Skincare" />
        </label>
        <label>
          Description
          <textarea name="description" rows={3} placeholder="A short description" />
        </label>
